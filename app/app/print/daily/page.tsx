...
export default async function PrintDailyPage({ searchParams }: { searchParams?: { date?: string } }) {
  ...
  return (
    <Card className="print-sheet">
      <CardHeader className="no-print flex-row items-center justify-between">
        <CardTitle>Today&apos;s Activities ({format(date, "PPP")})</CardTitle>
        <PrintButton />
      </CardHeader>
      <CardContent className="space-y-2 text-sm">
        {activities.length === 0 && <p>No activities scheduled.</p>}
