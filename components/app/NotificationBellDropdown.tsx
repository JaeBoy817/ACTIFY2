  ...
  return (
    ...
    >
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="relative inline-flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-300 bg-white text-zinc-800 transition hover:bg-zinc-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      ...
      <DropdownMenuContent align="end" className="w-[380px] border-zinc-200 bg-white p-0">
        <div className="flex items-center justify-between px-3 py-2.5">
          <DropdownMenuLabel className="p-0 text-sm">Notifications</DropdownMenuLabel>
          <div className="flex items-center gap-1.5">
            {loadingItems ? <Loader2 className="h-3.5 w-3.5 animate-spin text-muted-foreground" /> : null}
            <Badge variant="outline" className="border-zinc-300 bg-zinc-50 text-xs text-zinc-700">
              {unread} unread
            </Badge>
            <button
              type="button"
              className="inline-flex items-center gap-1 rounded-lg border border-zinc-300 bg-zinc-100 px-2 py-1 text-[11px] font-medium text-zinc-700 transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-50"
              onClick={() => {
        ...
        {loadingItems && items.length === 0 ? (
          ...
          <div className="max-h-[380px] overflow-auto p-1.5">
            {items.slice(0, 10).map((notification) => (
              <DropdownMenuItem key={notification.id} className="mb-1 h-auto items-start rounded-lg border border-transparent p-0 focus:bg-zinc-100">
                <div className="flex w-full items-start gap-2 p-2.5">
                  ...
                  {!notification.readAt ? (
                    <button
                      type="button"
                      className="mt-0.5 inline-flex shrink-0 items-center gap-1 rounded-md border border-zinc-300 bg-zinc-100 px-2 py-1 text-[11px] font-medium text-zinc-700 transition hover:bg-zinc-200"
                      onClick={(event) => {
